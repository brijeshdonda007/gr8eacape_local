...
class Helpsupport extends CI_Controller{
	public function __construct(){
		...
		$this->load->model('helpsupport_model');
		$this->load->database();
        $this->load->helper('text');
	}
	function index(){
        $data['user_profile_info']                      = $this->helpsupport_model->getUserInfo($this->session->userdata('user_id'));
        $data['helpsupport']                            =   $this->helpsupport_model->gethelpsupport(12);
        $data['main_client_view']                   =      'helpsupport/help-support';
