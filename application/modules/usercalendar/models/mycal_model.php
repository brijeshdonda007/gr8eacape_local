...
//		parent::Model();
		...
		$this->conf['template'] = '
			...
			{cal_cell_no_content}<div class="not-booked" id="click-date_{day}" onclick="reserveThisDate({day})"><div class="day_num">{day}</div><div >Reserve</div></div>{/cal_cell_no_content}
	...
	function generate ($year, $month) {
		...
		$private_code = $this->uri->segment(5);

                $property_rs = $this->getPropertyInfobyCode($private_code);
...

        {
            ...
            $data = array('start_date' => $start_date, 'end_date' => $start_date, 'property_id' => $property_id);
