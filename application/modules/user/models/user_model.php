...
class User_model extends MY_Model
{
    ...
    public function updateUserData($data)
    {
        ...
        $this->loadTable('tbl_users', 'id');

        (!empty($data['is_business']))? $data['is_business'] = (int)$data['is_business']:$data['is_business'] = 0;
        (!empty($data['gst_reg']))? $data['gst_reg'] = (int)$data['gst_reg']:$data['gst_reg'] = 0;

        return parent::update($data, $data['id']);
    }
	...
	*/
	...
	function update_property_details($certificate)
	{
		...
		if ($this->input->post('pet'))
			...
			$check_in_time = explode('/',$this->input->post('check_in_date')); 
			$check_in_date=$check_in_time[2]."-".$check_in_time[1]."-".$check_in_time[0]; 
			$check_out_time = explode('/',$this->input->post('check_out_date')); 
			$check_out_date=$check_out_time[2]."-".$check_out_time[1]."-".$check_out_time[0]; 
			
			//echo $this->input->post('standard_changed');die();
			@$standard_start_date='';
			@$standard_end_date='';
			@$winter_start_date='';
			@$winter_end_date='';
		...
		$data = array(
			...
			'summer_end_date' => $this->input->post('price_month'),
			...
			'region_active' => $this->input->post('region_active'),
		...
		if($this->input->post('property_id') != '') {
			...
			$property_sky_channel = $this->input->post('property_sky_channel');
			if(!empty($property_sky_channel)) {
				for ($i=0; $i < count($property_sky_channel); $i++){
					$data = array('property_id'=>$id, 'sky_channel_id'=>$property_sky_channel[$i]);
	...
	function getAllBokingRequest($user_id)
	{
		...
		$sql = "select a.*,b.title as prop_name, c.first_name as fname, c.last_name as lname, b.custom_url as url from tbl_property b inner join tbl_booking a
	...
	{
		$status_array = serialize(array('bb' => 2,'oo' => 2));
		$sql = "select a.*,b.title as prop_name,d.short_name, c.first_name as fname, c.last_name as lname, b.custom_url as url from tbl_property b inner join tbl_booking a
			on a.property_id = b.id inner join tbl_users c 
	...
	function getAllConfirmedBooking($user_id)
	{
		...
		$sql = "select a.*,b.title as prop_name, c.first_name as fname, c.last_name as lname, b.custom_url as url from tbl_property b LEFT join tbl_booking a
    ...
    /**
     * Confirmation email after owner confirm a escape for processing
     *
     *
     * @param $id
     */
    function confirmBookingByOwner($id)
	{
		...
		"Content-Type: text/html; charset=utf-8\r\n" .
		"Content-Transfer-Encoding: 8bit\r\n\r\n";
		$subject1='Process payment for '.$property->title.'!';
		$emailbody1 = '<p>Dear '.$buyer->first_name.' '.$buyer->last_name. ',</p><br/>';
		$emailbody1 .= '<p>Your booking reference no. '. $booking-> reference .' is accepted by owner. Please follow the link for payment: <a href="'. base_url().'booking/post_confirmation_process_payment/' . $id . '">'. base_url().'booking/post_confirmation_process_payment/' . $id . '</a> </p><br/>';
		$emailbody1 .= '<br/>';
		$emailbody1 .= 'Thank You<br/><strong>Support Team</strong><br/><strong>gr8 escapes</strong>';

        //emailLogger($buyer->email,$subject1,$emailbody1);
		@mail($buyer->email,$subject1,$emailbody1,$headers1);
	}
	...
	*/
	function saveInformation() {	
		...
		//////// check_in_time and check_out_time in YYYY-MM-DD fo storage in database
		
		if(isset($_POST['check_in_date']) && !empty($_POST['check_in_date'])) {
			$check_in_time = explode('/',$_POST['check_in_date']); 
			$check_in_date=$check_in_time[2]."-".$check_in_time[1]."-".$check_in_time[0];
		}
		if(isset($_POST['check_out_date']) && !empty($_POST['check_out_date'])) {
			$check_out_time = explode('/',$_POST['check_out_date']); 
			$check_out_date=$check_out_time[2]."-".$check_out_time[1]."-".$check_out_time[0];
		}
		//echo $_POST['standard_start_date'];die();
		...
		if($_POST['current_index']==0) {
			...
			if(isset($_POST['property_id']) && !empty($_POST['property_id']) ) {
				...
				$this->db->where(array('id' => $_POST['property_id']));
				$this->db->update('tbl_property', $data);
				echo "Information saved Successfully/Edit ";
			} elseif(isset($_POST['step1_save']) && $_POST['step1_save']==1 ) {
						
						$this->db->where('id', $_POST['save_property_id']);
						$this->db->update('tbl_property', $data);
								echo "updated";
				...
				if(isset($_POST['step1_save']) && $_POST['step1_save'] ==1) {
						$data = array(
							//'id' =>$_POST['save_escape_property_id'],
				...
				if(isset($_POST['property_id']) && !empty($_POST['property_id']) ) {
					...
					if($this->db->update('tbl_property', $data)) {
						...
						echo  "No Edit Information saved";
						 ...
						 $this->db->where('id', $_POST['save_property_id']);
						if($this->db->update('tbl_property', $data)){
								echo "Save/Edit Information saved";
			...
			print_r($_POST);die();
				...
				if(isset($_POST['property_id']) && !empty($_POST['property_id']) ) {
					...
					$this->db->where('id', $_POST['property_id']);
					if($this->db->update('tbl_property', $data)) {
							//echo " step 3 Edit Information saved";
					
					}else{
						//echo  "No step 3 Edit Information saved";
					} 
						 ...
						 $this->db->where('id', $_POST['save_property_id']);
						 if($this->db->update('tbl_property', $data)){
								//echo "Save/Edit step3 Information saved";
						
						}else{
							//echo "No Save/Edit step3 Information saved";
						}  
				 ...
				 if(isset($_POST['property_id']) && !empty($_POST['property_id']) ) {
									...
									if(isset($_POST['amenitiesArr']) && !empty($_POST['amenitiesArr'])) {
											for ($i=0; $i < count($_POST['amenitiesArr']); $i++){
												...
												[$i],$_POST['property_id']);
												 ...
												 if(empty($existing_amenities_id)) {
													...
													$this->db->insert('tbl_property_amenities', $dataArr);
									 ...
									 if(isset($_POST['amenitiesDeleteArr']) &&  !empty($_POST['amenitiesDeleteArr'])) {
										...
										if(isset($_POST['amenitiesDeleteArr']) &&  !empty($_POST['amenitiesDeleteArr'])) {
											for ($i=0; $i < count($_POST['amenitiesDeleteArr']); $i++){
												$dataArr = array('property_id'=>$_POST['save_property_id'], 'amenities_id'=>$_POST['amenitiesDeleteArr']
												[$i]);
								...
								if(!empty($_POST['categoryArr'])) {
									for ($i=0; $i < count($_POST['categoryArr']); $i++){
										...
										if(empty($existing_category_id)) {
											...
											$this->db->insert('tbl_property_cats', $dataArr);
										}	
									}
								}	
				 ...
				 if(isset($_POST['property_id']) && !empty($_POST['property_id']) ) {
								...
								if(!empty($_POST['skyChannelArr'])) {
									for ($i=0; $i < count($_POST['skyChannelArr']); $i++){
										...
										if(empty($existing_sky_channel_id)) {
											$dataArr = array('property_id'=>$_POST['property_id'], 'sky_channel_id'=>$_POST['skyChannelArr'][$i]);
											$this->db->insert('tbl_property_sky_channels', $dataArr);
										}
								...
								if(isset($_POST['skyChannelDeleteArr']) &&  !empty($_POST['skyChannelDeleteArr'])) {
									for ($i=0; $i < count($_POST['skyChannelDeleteArr']); $i++){
										...
										$this->db->where(array('property_id'=>$_POST['property_id'],'sky_channel_id' => $_POST['skyChannelDeleteArr'][$i]));
										$this->db->delete('tbl_property_sky_channels');
							 ...
							 $this->db->update('tbl_property', $data);
											...
											if(isset($_POST['skyChannelDeleteArr']) &&  !empty($_POST['skyChannelDeleteArr'])) {
												for ($i=0; $i < count($_POST['skyChannelDeleteArr']); $i++) {
													...
													$this->db->where(array('property_id'=>$_POST['save_property_id'],'sky_channel_id' => $_POST['skyChannelDeleteArr'][$i]));
													$this->db->delete('tbl_property_sky_channels');
							...
							if(!empty($_POST['skyChannelArr'])) {
								for ($i=0; $i < count($_POST['skyChannelArr']); $i++){
									$dataArr = array('property_id'=>$id, 'sky_channel_id'=>$_POST['skyChannelArr'][$i]);
