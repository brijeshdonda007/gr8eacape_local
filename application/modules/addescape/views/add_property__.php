...
<section id="content">
    <div class="wrapper clearfix">
		...
		<form id="form-3" method="post" class="profileform wizard" action="<?php echo site_url('Addescape/updateEscape'); ?><?php if(isset($property_code)) { echo "/".$property_code;}?>" enctype="multipart/form-data">
		...
		<input type="hidden" name="region_empty" id="region_empty" value="0" />
			...
			<!--	<?php $abd = @$property_info->mattress;?>
					...
					<!-- <fieldset id="cleaning_wrap" class="radio-fix">
					<label class="control-label">Cleaning:<span>*</span></label>
				<input type="radio" class="cleaning_escape" name="cleaner" id="no_cleaner" value="0">Leave it as you found it, spic & span,we don’t have a cleaner
					<input type="radio" name="cleaner" class="cleaning_escape" id="have_cleaner" value="1"> Leave it tidy, but no need to spring clean, we have a cleaner coming in after you leave
					<input type="radio" name="cleaner"  class="cleaning_escape" id="already_clean" value="1">No need to clean, this escape is cleaned daily 
				...
				<div id= "cleaning_wrap">
					<fieldset id="cleaning_id" class="radio-fix">
						<label class="control-label">Cleaning:<span>*</span></label>
						<input type="radio" name="cleaner"  class="cleaning_escape" id="no_cleaner" value="1" <?php if (@$property_info->cleaning == 1) echo 'checked="checked"'; ?> >  Leave it as you found it, spic & span,we don’t have a cleaner
					</fieldset>
					<fieldset class="radio-fix">
						<label class="control-label">&nbsp;</label>
						<input type="radio" name="cleaner"  class="cleaning_escape" id="have_cleaner" value="2" <?php if (@$property_info->cleaning == 2) echo 'checked="checked"'; ?> > Leave it tidy, but no need to spring clean, we have a cleaner coming in after you leave
					</fieldset>
					<fieldset  class="radio-fix">
						<label class="control-label">&nbsp;</label>
				...
				<div id="cleaning_charge_wrap">
					<fieldset id="cleaning_fee_id" class="radio-fix">
						<label class="control-label">Want to charge a separate cleaning fee to Guests ? <span>*</span></label>
				...
				<div id="cleaning_help_details">
					<fieldset id="cleaning_help_id" class="radio-fix">
						<label class="control-label">Should gr8 escapes need to arrange cleaning with your cleaner ? <span>*</span></label>
				...
				<div id= "bed_lines_wrap">
					<fieldset id="bed_lines_id" class="radio-fix">
						<label class="control-label">Bed Lines:<span>*</span></label>
					...
					<fieldset class="radio-fix">
						<label class="control-label">&nbsp;</label>
						<input type="radio" name="escape_bed_line"  class="bed_lines_escape" id="no_bed_lines" value="0" <?php if (@$property_info->bed_lines == '0') echo 'checked="checked"'; ?> > All provided for you, we will wash up after you have checked out
					</fieldset>
				</div>	
            ...
            <article>
				<fieldset id="amenities_wrap">
					<label class="control-label" style="margin-bottom:20px;">Facilities:<span>*</span></label>
					<div id="amenities_id">
            ...
            <article>
                <fieldset id="escape_category_wrap">
                    <label class="control-label" style="margin-bottom:20px;">Categories:<span>*</span></label>
                 <!--   <div class="property-category"> -->
                    <?php foreach($categories as $cat){ ?>
                  <!--       <fieldset id="category_checkbox"> -->
						<div style="width:33%;float:left;">
                          <input type="checkbox" 
						   id="<?php echo @$cat->id."_"."cat"; ?>" name="property_category[]" class="property_category" data-value="<?php echo $cat->category_title;?>" value="<?php echo $cat->id;?>" <?php if($this->uri->segment(2)){ if(in_array(@$cat->id, @$prop_cats)){ echo 'checked';}}?>><?php echo @$cat->category_title; ?>
						</div>
                  <!--       </fieldset> -->
                    <?php } ?>
                 <!--    </div> -->
			...
			<h3>Television</h3>
            <article>
				<label style="width:30px; !important ">Television</label>
                <fieldset id="escape_television_wrap">
                    <div class="property-television" style="margin-bottom:20px;margin-left:30px;">
					<select name="television_type_id" id="television_type_id" >
						<option value="0">Please Select</option>
					 <?php foreach($tvChannelsArray as $tvChannel){ ?>
						<option value="<?php echo @$tvChannel->id; ?>"><?php echo @$tvChannel->tv_category; ?></option>
					 <?php } ?>
					</select>
                    </div>
					<?php 
					foreach($channelsArray as $key => $value){
						foreach($channelsArray[$key] as $sky){ ?>
							<div class="activate_<?php echo $key ?> activated_tv_category" style="width:33%;float:left;display:none;">
            ...
            <article>
				...
				<fieldset>
					<label>Television Channels</label>
...
$(document).ready(function(){
	...
	var gallery_escape_id = $('#gallery_escape_id').val();
	 ...
	 $('.dropzone').on('click', function() {
	  ...
	  $('#television_type_id').on('change', function() {
		$(".error").hide();
		$('.activated_tv_category').hide();
		var selected_television_type = $("#television_type_id").val();
		$('.activate_'+selected_television_type).show();			
	  });
});
