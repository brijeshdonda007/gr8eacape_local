...
<div class="count_results"><img src="<?php echo base_url(); ?>assets/frontend/images/search-home.png" /><span class="result-first"><?php echo (($total_ajax_search > 0)?$total_ajax_search:'No');?> escapes found <?php if($this->input->post('searchq')) {?>in <span class="result-last"><?php echo $this->ajax_model->capitalizeWords($this->input->post('searchq'));?></span><?php }?></div>
...
<button id="loadmorex" class="btn btn-large btn-block buttonGreen" onclick="loadHomeSearchedData('<?php echo $loadCount;?>','<?php echo $this->input->post('searchq');?>')">Show me more accomodations <span class="flash"></span></button>
